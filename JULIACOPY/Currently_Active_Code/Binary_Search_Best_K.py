...
def output_results(epsilon, delta, test_num, n, filename, trials):
    ...
    copying results. Intended for use on different manipulations of
    N for now.

    epsilon = related to the accepted interval
    delta = related to the confidence interval
    test_num = number of tests to conduct
    n = actual set size
    ...
    for i in range(trials):
        line = str(binary_search_helper(epsilon, delta, test_num, n)) + "\n"
        file.write(line)
    
    now = datetime.datetime.now()
    end = time.time()
    file.write("\nDate and Time at program end: " + str(now))
    file.write("\nProgram Execution Time      : " + str(math.floor(end - start)) + " seconds")
